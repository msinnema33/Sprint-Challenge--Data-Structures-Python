...
'''
Solution using a Binary Search Tree == MY TRY #2 (put names_2 the list in the bst)
'''
# bst = BinarySearchTree('names')

# for name in names_2: # adds all names in names_2 to BST = O(n)
#     bst.insert(name)

# for name in names_1: # checks all names in names_1 to see if BST contains them = O(log n)
#     if bst.contains(name):
#         duplicates.append(name) # appends duplicates (BST.contains) to duplicates list
'''
# runtime after = 0.11200213432312012 seconds with 64 duplicates
# Complexity = O(nlog n)
'''
